  ...
  return (
    <div style={{fontFamily:SANS}}>
      ...
      {!loading && view === "bracket" && matches && (() => {
        ...
        // "COGNOME NOME - COGNOME NOME" -> "COGNOME / COGNOME"; BYE -> null
        ...
        const roundLabel = (round) => {
          if (/1\s*\u00b0?\s*[-\/]\s*2/.test(round || "")) return "Finale";
          return (round || "").replace(/\s*Vincenti/i, "").trim() || "Turno";
        };
        const BOX_H = 46;
        const colHeight = (cols[0] ? cols[0].ms.length : 4) * (BOX_H + 12);
        const MatchBox = ({ m }) => {
          ...
          return (
            <div style={{background:A.card,border:`1px solid ${A.line}`,borderRadius:8,padding:"5px 8px",width:134,boxSizing:"border-box",fontSize:10}}>
              <div style={{display:"flex",justifyContent:"space-between",gap:4,color:aS?(aWin?A.accent:A.text):A.soft,fontWeight:aWin?700:400}}>
              ...
              <div style={{height:1,background:A.line,margin:"3px 0"}}/>
              <div style={{display:"flex",justifyContent:"space-between",gap:4,color:bS?(bWin?A.accent:A.text):A.soft,fontWeight:bWin?700:400}}>
          ...
          <div>
              ...
              <div style={{display:"flex",gap:16,minWidth:"min-content"}}>
                {cols.map((col, ci) => (
                  <div key={ci} style={{display:"flex",flexDirection:"column"}}>
                    <div style={{fontSize:10,fontWeight:700,color:A.sub,textAlign:"center",marginBottom:8,textTransform:"uppercase",letterSpacing:0.5,whiteSpace:"nowrap"}}>
                      {roundLabel(col.round)}
                    </div>
                    <div style={{display:"flex",flexDirection:"column",justifyContent:"space-around",height:colHeight}}>
                      {col.ms.map((m, mi) => <MatchBox key={mi} m={m}/>)}
                    </div>
                  </div>
                ))}
              </div>
            ...
            {f34 && (
              <div style={{marginTop:16,borderTop:`1px solid ${A.line}`,paddingTop:14}}>
                <div style={{fontSize:10,fontWeight:700,color:A.sub,marginBottom:8,textTransform:"uppercase",letterSpacing:0.5}}>Finale 3°/4° posto</div>
                <div style={{maxWidth:150}}>{f34.ms.map((m, mi) => <MatchBox key={mi} m={m}/>)}</div>
              </div>
            )}
            <div style={{fontSize:10,color:A.soft,marginTop:10,textAlign:"center"}}>Scorri → per vedere tutti i turni</div>
          </div>
        );
      })()}
