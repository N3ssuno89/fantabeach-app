...
exports.handler = async (event) => {
  ...
  try {
    ...
    // ── Costruisce mappa nome → {id, gender, surname} da PLAYER_MAPPING ──
    const nameToPlayer = {};
    const idToSurname = {};
    (mappingRes.data.values || []).slice(1).forEach(row => {
      ...
      const nome    = row[3]?.trim() || ""; // colonna Nome (indice 3)
      const cognome = row[4]?.trim() || ""; // colonna Cognome (indice 4)
      if (id && name) {
        nameToPlayer[normalizeName(name)] = { id, gender };
        if (cognome) idToSurname[id] = cognome;
      }
    });
